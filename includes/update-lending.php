...
if (isset($_POST['expense-id'])) {
  ...
  $date = isset($_POST['date_of_cho vay']) ? $_POST['date_of_cho vay'] : '';
  $description = isset($_POST['description']) ? $_POST['description'] : '';
  $amount = isset($_POST['amount']) ? $_POST['amount'] : '';
  $status = isset($_POST['status']) ? $_POST['status'] : '';

  // Prepare and execute the SQL query to update the expense in the database
  $stmt = $db->prepare('UPDATE cho vay SET name = ?, date_of_cho vay = ?, description = ?, amount = ?, status = ? WHERE id = ? AND UserId = ?');
  ...
  if ($result) {
    $message = "Cho vay updated successfully";
    echo "<script type='text/javascript'>alert('$message');</script>";
    echo " <script type='text/javascript'>window.location.href = 'manage-cho vay.php';</script>";
    exit();
  } else {
    // If the update failed, redirect to the edit expense page with an error message
    $message = "Please try again !";
    echo "<script type='text/javascript'>alert('$message');</script>";
    echo " <script type='text/javascript'>window.location.href = 'manage-cho vay.php?id=' . $id';</script>";
    exit();
  }
}
?>
